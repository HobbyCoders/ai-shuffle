 ...
 */
function getReasoningProvider(): { providerId: string; model: string; credentials: ProviderCredentials } | null {
 ...
 */
export async function aiReason(input: ReasonInput): Promise<ReasonResult> {
...
function estimateConfidence(text: string): number {
  ...
  if (lowConfidence.test(text)) return 0.4;
  if (mediumConfidence.test(text)) return 0.65;
 ...
 */
export async function checkLogic(argument: string): Promise<{
  valid: boolean;
  flaws: string[];
  confidence: number;
}> {
 ...
 */
export async function reasonedAnswer(
  question: string,
  context?: string
): Promise<{
  answer: string;

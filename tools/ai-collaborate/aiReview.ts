  ...
  try {
    ...
    try {
      ...
      return {
        ...
        concerns: (parsed.concerns || []).map((c: any): ReviewConcern => ({
...
export async function reviewFile(
  filePath: string,
  options?: Omit<ReviewInput, 'content'>
  ...
  try {
    ...
    let review_type: ReviewType = options?.review_type || 'general';
 ...
 */
export async function securityReview(content: string, context?: string): Promise<ReviewResult> {
  return aiReview({
    content,
 ...
 */
export async function multiReview(
  content: string,
  review_type: ReviewType = 'code',
  providers: string[] = ['openai', 'google']
): Promise<{
  success: boolean;
  reviews: Array<{ provider: string; result: ReviewResult }>;
  consensus?: {
    approved: boolean;
  ...
  const criticalCount = successfulReviews.reduce(
    (sum, r) => sum + r.result.concerns.filter(c => c.severity === 'critical').length,
    0
  );
  const majorCount = successfulReviews.reduce(
    (sum, r) => sum + r.result.concerns.filter(c => c.severity === 'major').length,
    0
  );

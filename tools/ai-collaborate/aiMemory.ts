...
function getCredentials(): ProviderCredentials | null {
 ...
 */
async function recallMemory(query: string, limit: number = 10): Promise<MemoryResult> {
  const store = loadMemory();
 ...
 */
function simpleRecall(query: string, store: MemoryStore, limit: number): MemoryResult {
 ...
 */
function searchByTags(tags: string[], store: MemoryStore, limit: number): MemoryResult {
 ...
 */
async function summarizeMemories(): Promise<MemoryResult> {
  const store = loadMemory();
 ...
 * Clear all memories
 */
function clearMemories(): MemoryResult {
  const store = loadMemory();
 ...
 */
export async function aiMemory(input: MemoryInput): Promise<MemoryResult> {
  switch (input.action) {
    case 'store':
      if (!input.content) {

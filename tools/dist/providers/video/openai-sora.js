 ...
 */
async function pollForCompletion(videoId, apiKey, maxPollTime = 10 * 60 * 1000, // 10 minutes for Sora
...
function saveVideo(videoBuffer, prefix, duration) {
 ...
 */
function aspectRatioToSize(aspectRatio = '16:9', resolution = '720p') {
 ...
 */
function normalizeSeconds(duration = 8) {
...
// ============================================================================
export const openaiSoraProvider = {
    id: 'openai-sora',
    name: 'OpenAI Sora',
    models: [
        {
    ...
    async validateCredentials(credentials) {
        try {
            ...
            const hasSora = models.some((m) => m.id?.includes('sora') || m.id?.includes('video'));
            if (!hasSora) {
                return {
                    valid: true,
...
//# sourceMappingURL=openai-sora.js.map
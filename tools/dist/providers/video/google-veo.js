...
async function pollForCompletion(operationName, apiKey, maxPollTime = 6 * 60 * 1000, pollInterval = 5000) {
...
async function downloadVideo(videoUri, apiKey) {
...
function saveVideo(videoBuffer, prefix, duration, sourceVideoUri) {
...
// ============================================================================
export const googleVeoProvider = {
    id: 'google-veo',
    name: 'Google Veo',
    models: [
        {
    ...
    async generate(input, credentials, model) {
        ...
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:predictLongRunning?key=${apiKey}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    instances: [{ prompt }],
    ...
    async imageToVideo(input, credentials, model) {
        const apiKey = credentials.apiKey;
        // Validate input
        if (!input.image_path) {
    ...
    async extend(input, credentials, model) {
        const apiKey = credentials.apiKey;
        // Validate input
    ...
    async bridgeFrames(input, credentials, model) {
        const apiKey = credentials.apiKey;
        // Validate input
    ...
    async validateCredentials(credentials) {
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${credentials.apiKey}`);
...
//# sourceMappingURL=google-veo.js.map
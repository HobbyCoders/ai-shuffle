 ...
 */
async function uploadVideoFile(filePath, apiKey) {
    try {
 ...
 */
async function waitForFileProcessing(fileName, apiKey, maxWaitTime = 5 * 60 * 1000) {
    const startTime = Date.now();
    while (Date.now() - startTime < maxWaitTime) {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/${fileName}?key=${apiKey}`, { method: 'GET' });
        if (!response.ok) {
            await sleep(3000);
            continue;
        }
...
// ============================================================================
export const googleGeminiVideoProvider = {
    ...
    async analyzeVideo(input, credentials, model) {
...
//# sourceMappingURL=google-gemini-video.js.map
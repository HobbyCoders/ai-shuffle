...
// Helpers
// ============================================================================
function getGeneratedModelsDir() {
...
async function meshyFetch(endpoint, apiKey, options = {}) {
...
async function pollForCompletion(taskId, taskType, apiKey, maxPollTime = MAX_POLL_TIME, pollInterval = DEFAULT_POLL_INTERVAL) {
...
function parseTaskResult(taskResult, taskType) {
    const result = {
        success: true,
    ...
    if (taskResult.model_urls || taskResult.model_url) {
        ...
        if (taskResult.glb_url)
            result.model_urls.glb = taskResult.glb_url;
        if (taskResult.fbx_url)
            result.model_urls.fbx = taskResult.fbx_url;
        if (taskResult.obj_url)
            result.model_urls.obj = taskResult.obj_url;
        if (taskResult.usdz_url)
            result.model_urls.usdz = taskResult.usdz_url;
    }
...
async function downloadAndSaveModel(modelUrl, apiKey, prefix, format = 'glb') {
    try {
...
export const meshyProvider = {
    ...
    async textTo3D(input, credentials, model) {
    ...
    async imageTo3D(input, credentials, model) {
        const apiKey = credentials.apiKey;
        // Validate input
        if (!input.image_path) {
        ...
        try {
            ...
            if (input.should_texture !== undefined)
                requestBody.should_texture = input.should_texture;
            if (input.enable_pbr !== undefined)
                requestBody.enable_pbr = input.enable_pbr;
            if (input.pose_mode)
                requestBody.pose_mode = input.pose_mode;
            if (input.texture_prompt)
                requestBody.texture_prompt = input.texture_prompt;
    ...
    async retexture(input, credentials, model) {
        const apiKey = credentials.apiKey;
        // Validate input
        ...
        try {
            ...
            if (input.style_prompt)
                requestBody.text_style_prompt = input.style_prompt;
            if (input.style_image_url)
                requestBody.image_style_url = input.style_image_url;
            if (input.enable_original_uv !== undefined)
                requestBody.enable_original_uv = input.enable_original_uv;
            if (input.enable_pbr !== undefined)
                requestBody.enable_pbr = input.enable_pbr;
    ...
    async rig(input, credentials) {
        ...
        try {
            ...
            if (input.height_meters !== undefined)
                requestBody.height_meters = input.height_meters;
            if (input.texture_image_url)
                requestBody.texture_image_url = input.texture_image_url;
    ...
    async animate(input, credentials) {
        const apiKey = credentials.apiKey;
        // Validate input
        ...
        try {
            ...
            if (input.operation_type)
                requestBody.operation_type = input.operation_type;
    ...
    async getTask(taskId, taskType, credentials) {
        const apiKey = credentials.apiKey;
        if (!taskId) {

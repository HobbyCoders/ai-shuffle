...
{#if visible && (filteredFiles.length > 0 || loading)}
  <div class="absolute bottom-full left-0 right-0 mb-2 bg-popover border border-border rounded-lg shadow-lg max-h-64 overflow-hidden z-50">
    ...
    <div class="px-3 py-1.5 border-t border-border bg-muted/30 text-xs text-muted-foreground">
      <kbd class="px-1 py-0.5 bg-muted rounded mr-1">Tab</kbd> navigate
      <kbd class="px-1 py-0.5 bg-muted rounded mx-1">Enter</kbd> select
      <kbd class="px-1 py-0.5 bg-muted rounded mx-1">Esc</kbd> close
    </div>
  </div>
{/if}

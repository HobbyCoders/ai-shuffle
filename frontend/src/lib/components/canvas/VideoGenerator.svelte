<script lang="ts">
	...
	const providers = [
		{ id: 'google-veo', name: 'Veo', maxDuration: 8, durations: [4, 6, 8] },
		{ id: 'openai-sora', name: 'Sora', maxDuration: 12, durations: [4, 6, 8, 12] }
	];

	const aspectRatios = ['16:9', '9:16', '1:1'];

	$: currentProvider = providers.find(p => p.id === provider);
	$: availableDurations = currentProvider?.durations || [4, 6, 8];
...
<div class="max-w-2xl mx-auto space-y-6">
	<!-- Prompt Input -->
	<div>
		<label class="block text-sm font-medium text-foreground mb-2">Describe your video</label>
		<textarea
			bind:value={prompt}
			...
			<div class="flex gap-2">
				{#each availableDurations as dur}
					<button
						...
						class="flex-1 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {duration === dur ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground hover:bg-hover-overlay border border-border'}"
					>
						{dur}s
					</button>
				{/each}
	...
	<div>
		<label class="block text-xs text-muted-foreground mb-2">Source Image (optional)</label>
